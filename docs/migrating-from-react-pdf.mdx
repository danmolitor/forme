...
> **Moving off react-pdf but want to switch to Svelte at the same time?** The [Svelte adapter](/svelte) exposes the same components with the same props — the rewrites in the tables below apply verbatim, just import from `@formepdf/svelte` instead of `@formepdf/react` and author templates as `.svelte` files.

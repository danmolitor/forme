...
description: How to migrate from react-pdf to Forme.
---
...
1. **Links.** react-pdf supports clickable links with `<Link>`. Forme does not support PDF link annotations yet.

2. **Absolute positioning.** react-pdf supports `position: "absolute"`. Forme only supports fixed headers/footers via `<Fixed>`, not arbitrary absolute positioning.

3. **Bookmarks/outlines.** react-pdf can generate PDF bookmarks. Forme does not support this yet.

4. **Text run styling.** react-pdf supports inline styling within text (e.g., bold a single word). Forme treats each `<Text>` element as a single styled run.

5. **Emoji rendering.** react-pdf handles emoji via system fonts. Forme does not have special emoji support.

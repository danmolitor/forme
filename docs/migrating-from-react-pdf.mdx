...
> **Moving off react-pdf but want to switch to Svelte or Vue at the same time?** The [Svelte](/svelte) and [Vue](/vue) adapters expose the same components with the same props — the rewrites in the tables below apply verbatim, just import from `@formepdf/svelte` or `@formepdf/vue` instead of `@formepdf/react` and author templates as `.svelte` or `.vue` files.
...
| `<Canvas>` | `<Canvas>` | Same. Pass a `draw` callback that receives a recording context; operations are handed to the engine. |
...
4. **Speed.** Forme's Rust/WASM engine renders typical documents in roughly 15-25ms and cold-starts in tens of milliseconds; see the measured [benchmarks](https://parity.formepdf.com/#benchmarks). (We benchmark against Puppeteer on identical HTML; we have not run a controlled react-pdf comparison, so we don't quote a multiplier.)

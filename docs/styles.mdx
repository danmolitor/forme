...
| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `position` | `"relative"` \| `"absolute"` | `"relative"` | Positioning mode. `relative` keeps the element in normal flow and paints it offset by `top`/`left`/`right`/`bottom` (siblings don't move). `absolute` takes it out of flow and places it against its **nearest positioned ancestor** — an element with `position: 'relative'`/`'absolute'` — or the page content box if there is none. |
| `top` | `number` | - | Offset from the top of the containing block (nearest positioned ancestor, or the page) |
| `right` | `number` | - | Offset from the right of the containing block |
| `bottom` | `number` | - | Offset from the bottom of the containing block |
| `left` | `number` | - | Offset from the left of the containing block |

### Positioning examples

```tsx
{/* Badge overlay in the top-right corner — the card is position: 'relative'
    so the badge resolves against it, not the page. */}
<View style={{ position: 'relative', width: 200, height: 100, backgroundColor: '#f8fafc' }}>

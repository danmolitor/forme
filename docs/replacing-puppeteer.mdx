...
description: Replace Puppeteer and headless Chrome PDF generation with Forme. 100x faster, no browser dependency, reliable page breaks.
---
...
**Puppeteer PDF generation is slow.** Launching Chrome, creating a page, setting HTML content, and rendering to PDF typically takes 1-3 seconds. Forme renders the same document in 5-15ms because there is no browser to start.

**Chrome uses too much memory.** Each Chrome process consumes 50-200MB of RAM. Under concurrent load, memory pressure leads to crashes and OOM kills. Forme uses 2-5MB per render.

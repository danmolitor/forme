...
> **Svelte users:** every component below is also exported from `@formepdf/svelte` with identical props. Swap the import path and the JSX examples work as `.svelte` markup unchanged. See the [Svelte guide](/svelte) for setup.

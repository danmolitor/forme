...
The adapter requires Preact 10 (`^10.19.0`) as a peer dependency. `@formepdf/core` is an *optional* peer: it is only needed to render PDF bytes locally (`renderDocument`). If you only serialize templates to JSON (for a queue, a self-hosted server, or another runtime), skip it — `serialize` works with zero WASM.
...
The compiled-template system (`$ref`, `$each`, `$if`, expression helpers) works identically to `@formepdf/react` because the recording-proxy layer is framework-agnostic. The workflow is the same regardless of which authoring adapter you use.

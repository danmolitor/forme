...
pub fn subset_ttf(
    ...
    let raw_glyf = find_table(ttf_data, b"glyf").ok_or("Missing glyf table")?;
    let raw_loca = find_table(ttf_data, b"loca").ok_or("Missing loca table")?;
    let head = find_table(ttf_data, b"head").ok_or("Missing head table")?;
    ...
    let (new_glyf, new_loca_offsets) =
        rebuild_glyf(raw_glyf, &loca_offsets, &needed_gids, &gid_remap);
    ...
    let raw_hmtx = find_table(ttf_data, b"hmtx").ok_or("Missing hmtx table")?;
    let raw_hhea = find_table(ttf_data, b"hhea").ok_or("Missing hhea table")?;
    ...
    let new_os2 = find_table(ttf_data, b"OS/2").map(|t| t.to_vec());
    ...
    if let Some(cvt) = cvt_data {
        tables.push((tag_u32(b"cvt "), cvt));
    }
    if let Some(fpgm) = fpgm_data {
        tables.push((tag_u32(b"fpgm"), fpgm));
    }
    ...
    if let Some(os2) = new_os2 {
        tables.push((tag_u32(b"OS/2"), os2));
    }
    tables.push((tag_u32(b"post"), new_post));
    if let Some(prep) = prep_data {
        tables.push((tag_u32(b"prep"), prep));
    }
...
fn collect_composite_deps(glyf: &[u8], loca_offsets: &[u32], gid: u16, needed: &mut BTreeSet<u16>) {
...
fn rebuild_hmtx(hmtx: &[u8], needed_gids: &BTreeSet<u16>, num_h_metrics: usize) -> Vec<u8> {
    let mut data = Vec::new();
...
fn build_cmap_format4(char_to_gid: &[(u16, u16)]) -> Vec<u8> {
    ...
    let entry_selector = if seg_count > 0 {
        (seg_count as f64).log2().floor() as u16
    } else {
        0
    };
    ...
    for i in 0..num_tables {
        ...
        if tag == head_tag {
            ...
            if table_offset + head_data_len <= output.len() {
                let checksum =
                    calc_table_checksum(&output[table_offset..table_offset + head_data_len]);

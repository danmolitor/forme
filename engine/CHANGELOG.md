...
- **Half-leading render-defect warning.** Glyphs currently sit at the top of their line box rather than vertically centered in it (half-leading is not applied — a known structural limitation). At `line-height >= 2× font-size` this is almost always the pre-flexbox vertical-centering idiom, so it now reports itself: `render defect: text renders at the top of its Npt line box — line-height half-leading is not applied; center with flex align-items instead`.
...
- **Absolutely-positioned children no longer counted in auto heights.** `measure_children_height` summed absolute children into the flow height while `layout_children` takes them out of flow — every auto-height ancestor of an absolutely-positioned element reserved phantom space equal to its height. Fifth instance.
- **Image sizing: one ladder for measure and layout, Chrome semantics.** Measurement sized an undimensioned image at container width × aspect while layout drew it at intrinsic size — a small logo styled `width:100%; max-width:300px` reserved a container-sized phantom block and rendered at a few points (the single biggest whitespace cause in the template corpus). Both paths now share `image_display_size`: style width (percents resolved against the container) > explicit props > intrinsic, `max-width`/`min-width` clamp, height from the real aspect ratio unless given — matching Chrome. Sixth instance of the measure/layout family, and the reason the two paths are now one function.
- **Unfittable table rows no longer emit blank pages.** A row taller than any page (the email-template idiom: everything in one `<tr>`) forced a page break even at the top of an empty page and then stranded its container on a visually blank trailing page. Breaks now happen only when a fresh page actually buys room; an overflowing row's wrapper lands on the first page of its own content; and the case reports itself: `render defect: table row needs Npt but a page holds Mpt — rows are atomic, so it is placed whole and overflows`.

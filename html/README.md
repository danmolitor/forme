...
| `box-sizing` — `content-box` (the CSS default) grows point-valued `width`/`height`/`min-*`/`max-*` by padding + border on the way to the engine (whose fixed dimension is always the border box); a declared `border-box` passes values through untouched, so Bootstrap-reset documents render as before | `box-sizing` on percentage dimensions — a "percent of parent content plus points" box isn't expressible in the engine's dimension model: border-box percent is exact, content-box percent-with-padding keeps the historical approximation |
...
| `text-align` (incl. `justify` — Knuth-Plass + real inter-word distribution), `text-decoration`, `text-transform` (Unicode-aware), `letter-spacing`, `word-spacing` (block-level; applied through the PDF `Tw` operator, so it spaces the rendered words but is **not** part of text measurement, and lines break as though it were absent) | percentage margins/padding (warned, treated as 0) |
...
| `display: block / flex / none`, `flex-direction`, `justify-content`, `align-items` (including `baseline` — pending, currently top-aligns), `gap`, `flex-grow`, `flex-shrink`, `flex: <n>` (single-number shorthand) | `flex-wrap`, `flex-basis`, `flex: none` and multi-value `flex` shorthands — warn by name |

...
function exportedComponents(): [string, React.ElementType][] {
  const skip = new Set([
  ...
  return Object.entries(Forme as Record<string, unknown>)
    ...
    .map(([name, v]) => [name, v as React.ElementType]);
}
 ...
 */
function sample(name: string, C: React.ElementType): React.ReactElement {
...
function contribution(
  ...
  isPage: boolean,
  serializer: (e: React.ReactElement) => unknown,
): number {
...
describe('serialize() and serializeTemplate() agree about every exported component', () => {
  ...
  for (const [name, C] of components) {
    it(`${name} survives both paths`, () => {
      ...
      const viaSerialize =
        contribution(body, isPage, Forme.serialize as (e: React.ReactElement) => unknown) > 0;
      const viaTemplate =
        contribution(body, isPage, Forme.serializeTemplate as (e: React.ReactElement) => unknown) > 0;

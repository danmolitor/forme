...
- **CSS tables: `display: table` / `table-row` / `table-cell` on divs** — the pre-flexbox equal-height-columns idiom. Multi-row CSS tables map onto the engine's native table machinery (anonymous-row rules for loose cells, row groups transparent, column widths harvested from the first row). A **single-row** CSS table is the columns idiom and maps to a flex row instead — breakable across pages, since engine table rows are atomic and real templates wrap arbitrarily tall content in one such row (the compat corpus's template 09 wraps its **entire document** in one). Bootstrap's print-stylesheet display values (`table-header-group`, `table-cell` on real table tags) no longer produce warning noise. Known boundary, reported through the render-defect channel: a flex row that splits across pages lays its children **sequentially** — columns taller than a page do not yet continue side by side (template 09 renders complete and legible, sidebar pages then content pages, but not Chrome's parallel columns).

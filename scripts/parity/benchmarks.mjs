...
const WHERE_WE_LOSE = {
  ...
  trackedFixes: [
    ...
    { name: 'Reduce per-node allocation (arena / SmallVec / drop style clones)', target: 'throughput', note: 'dhat-profiled 2026-09 (benchmarks/harness/dhatprofile + dhat-top.mjs). PARTIAL FIX LANDED: PositionedGlyph.font_family String→Arc<str>. It was the #1 site by allocation count — every glyph owned its own family string, so each of the millions of glyph clones (TextLine/subtree clones, measure trial-layouts) heap-allocated one. Arc<str> makes clone a refcount bump. Result: allocations −31% (ledger-500p 182.8M→125.0M, invoice-50p 9.24M→6.39M), output byte-identical, all gates green. REMAINING (bytes, not count): ~60% of allocated bytes is LayoutElement::clone deep-copying children: Vec<LayoutElement> subtrees, driven by measure_node_height trial-layout and pagination rollback — an algorithmic change (measure on borrowed nodes / index-based rollback) that needs its own proposal.' },

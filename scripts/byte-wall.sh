...
if [ ${#FIXTURES[@]} -eq 0 ]; then
  ...
  # would have spoken.
  FIXTURES=(letterhead report zebra-invoice statement dashed-borders invoice styled-text-blocks fragmented-columns wrapped-float-rows auto-margin-container fixed-height-badges flex-margin-space)
fi
